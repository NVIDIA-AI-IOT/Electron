...
def interpret_output(output, img_width, img_height):
	...
	num_box = 2
	grid_size = 7
	probs = np.zeros((7,7,2,1))
	class_probs = np.reshape(output[0:980],(7,7,1))
#	print class_probs
...
def main(argv):
	...
	try:
		...
		print 'detect.py -m <model_file> -w <output_file> -i <img_file>'
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print 'detect.py -m <model_file> -w <weight_file> -i <img_file>'
			sys.exit()
		elif opt == "-m":
			model_filename = arg
		elif opt == "-w":
	...
	print 'total time is " milliseconds', elapsedTime.total_seconds()*1000, out
	print out#.iteritems()
	img_cv = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
	results = interpret_output(out['results'][0], img.shape[1], img.shape[0])
